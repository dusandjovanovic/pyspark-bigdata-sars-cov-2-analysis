...
def main():
    spark = SparkSession.builder.appName('cases_clinical_spectrum_analysis').getOrCreate()

    # extracting and transforming the dataset
    data = extract_data(spark)
    data_initial = transform_data(data)
...
def transform_age_relations(dataframe):
    dataframe_clean = dataframe.fillna(0)
    dataframe_clean = dataframe_clean.replace("nan", "0")
...
def transform_care_relations(dataframe):
    dataframe_clean = dataframe.fillna(0)
    dataframe_clean = dataframe_clean.replace("nan", "0")
...
def transform_predictions_value_distribution(dataframe):
    df_transformed = dismiss_missing_values(dataframe)

    return df_transformed


def transform_predictions_test_result_distribution(dataframe):
    df_transformed = dismiss_missing_values(dataframe)
